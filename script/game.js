...
class game {
	...
	//видалення пустих строк з ігрового поля
	static remove_rows(){
		let p, row = 0;
		while(row < g_row2){
			p = 0;
	...
	static on_click(e){
		...
		case STATE.TITLE:
			...
			case 2:
				{
					let p = prompt("Змінити швидкість падіння (максимально до 10)", g_ivel);
					if((p != null) && !isNaN(p)){
...
//проверка на перевертання фігури
...
//показ ігрового вікна та панелі управління
const show_game = () => {
...
const text_level = (n) => {
	set_text(g_tlevel, "Рівень - " + (n + 1));
}

const text_lines = (n) => {
	set_text(g_tlines, "Ліній - " + n);
}
