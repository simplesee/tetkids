...
function show_menu(){
	...
	g_inf.hdc.fillStyle = "#FF00FF";

	for(let i = 3; i >= 0; --i)
		g_inf.hdc.fillText(str, (w - sz.width)/2 - i, (h - cy)/4 - i);
	g_inf.hdc.fillStyle = "#C71585";
	...
	g_inf.hdc.fillStyle   = "#800080";
	g_inf.hdc.strokeStyle = "#FFFF00";
	g_inf.hdc.shadowBlur  = Math.ceil(sep * 0.5);
	g_inf.hdc.shadowColor = "#FF1493";
	let p;
	...
	cy = Math.ceil(w * 0.05);
	g_inf.hdc.font = cy + FONT_FACE;
	g_inf.hdc.fillStyle   = "#FFFF00";
	g_inf.hdc.strokeStyle = "#FF4500";

	top = Math.floor(h / 3);
	const bs = ["Почати гру", "Управління", "Швидкість"];
	let m;
...
function show_help(){
	...
	const hs = "Управління грою за допомогою правої панелі";
	...
	const cs = [
		...
		"Рух вліво - кнопка вліво або A", 
		"Рух вправо - кнопка вправо або D", 
		"Рух до низу - кнопка вниз або S", 
		"Обертання - кнопка вверх або W"
	];
